...
let opponent = await inquirer.prompt({
  ...
  choices: ["Skeleton", "batman", "Zombie"],
});
console.log(
  `${chalk.bold.blue(player.name)} VS ${chalk.bold.blue(opponent.select)}`
);
...
do {
  if (
    opponent.select === "Skeleton" ||
    opponent.select === "batman" ||
    ...
    if (ask.option === "Attack") {
      ...
      if (number <= 0) {
        ...
        if (p1.fuel <= 0) {
          console.log(
            chalk.bold.blue(`${p1.name} Lose! Better luck next time.`)
          );
          process.exit();
        }
      }
    ...
    if (ask.option === "Run for life...") {
      console.log(chalk.bold.green(`${p1.name} Lose! Better luck next time.`));
      process.exit();
    }
  }
} while (true);